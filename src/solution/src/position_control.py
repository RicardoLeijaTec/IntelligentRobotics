...
def actuate():
    ...
    global robot_state, target
    global actual_position_pub, actual_pos

    e_theta = atan2(target['y'], target['x']) - theta_k
    ...
    d_t = math.sqrt((target['x'])**2 + (target['y']) **2)  # Distancia total por recorrer
    e_d = math.sqrt((target['x'] - xk)**2 + (target['y'] - yk)**2)

    lost = atan2(target['y'] - yk, target['x'] - xk)*180/pi
...
def main():
    ...
    rospy.Subscriber("/target_position", Pose, set_target)
